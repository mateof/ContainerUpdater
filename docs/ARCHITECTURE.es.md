...
**Compose** cuando el YAML es accesible. `spawn` con `shell: false`, nombres
...
Los proyectos creados desde la aplicación se anotan además en la base de datos.
Es lo que mantiene visible uno que todavía no tiene contenedores, que es justo
cuando hace falta poder volver a él para corregir el YAML.
...
El mismo sobre protege las versiones archivadas de los ficheros de proyecto. La
copia de seguridad de un secreto sigue siendo un secreto, y un `.env` lleva
contraseñas de bases de datos y tokens de API.
